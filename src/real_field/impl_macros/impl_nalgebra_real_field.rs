/// Implement the [`nalgebra::RealField`](https://docs.rs/nalgebra/latest/nalgebra/trait.RealField.html)trait for a type that has already implemented
/// [`crate::real_field::RealFieldBase`].
///
/// [`nalgebra::RealField`](https://docs.rs/nalgebra/latest/nalgebra/trait.RealField.html)requires [`num_traits::Num`], [`num_traits::Signed`], and
/// [`num_traits::FromPrimitive`] as supertraits, so `$t` must already implement all three (e.g. via
/// [`crate::impl_num_traits_num`], [`crate::impl_num_traits_signed`], and
/// [`crate::impl_num_traits_from_primitive`]) before invoking this macro. In most cases, prefer
/// [`crate::impl_real_field`] instead, which takes care of this for you.
///
/// # Generic Arguments
///
/// * `$t` - The type for which the [`nalgebra::RealField`](https://docs.rs/nalgebra/latest/nalgebra/trait.RealField.html)trait is being implemented.
#[doc(hidden)]
#[macro_export]
macro_rules! impl_nalgebra_real_field {
    ($t:ty) => {
        use $crate::__private::{approx, nalgebra, simba};

...
fn test_vec() {
    ...
    let mat: Mat<f64> = vec.new_matrix_m_by_n::<0>(Some(M));
    let expected: Mat<f64> = Mat::new_with_shape(M, N);
...
fn test_nalgebra_dmatrix() {
    ...
    let expected: DMatrix<f64> = DMatrix::new_with_shape(M, N);
    assert_arrays_equal!(mat, expected);
    assert_eq!(mat.shape(), (M, N));
}
...
fn test_nalgebra_smatrix() {
    ...
    let expected: SMatrix<f64, M, N> = SMatrix::new_with_shape(M, N);
    assert_arrays_equal!(mat, expected);
    assert_eq!(mat.shape(), (M, N));
}
...
fn test_ndarray() {
    ...
    let mat: Array2<f64> = vec.new_matrix_m_by_n::<0>(Some(M));
    let expected: Array2<f64> = Array2::new_with_shape(M, N);
    assert_arrays_equal!(mat, expected);
    assert_eq!(Matrix::shape(&mat), (M, N));
}
...
fn test_faer() {
    ...
    let mat: FMat<f64> = vec.new_matrix_m_by_n::<0>(Some(M));
    let expected: FMat<f64> = FMat::new_with_shape(M, N);

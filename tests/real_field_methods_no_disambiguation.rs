...
//! `RealFieldBase`, `num_traits::Zero`, and `num_traits::One`) can be called on a generic
//! `T: RealField` without needing to disambiguate between traits.
...
fn call_all_real_field_methods<T: RealField>(a: T, b: T, c: T) {
    ...
    // `num_traits::Zero` (no explicit import needed since it's a supertrait of `RealField`).
    let mut z = a;
    let _ = T::zero();
    let _ = a.is_zero();
    z.set_zero();

    // `num_traits::One` (no explicit import needed since it's a supertrait of `RealField`).
    let mut o = a;
    let _ = T::one();
    let _ = a.is_one();
    o.set_one();
